...
#After a block falls on the game board, 
#the position of the tiles where that block falls in the game board, 
#will show the same color as the falling block itself.
...
#Change the possible shape of each drop-down block
...
#The space is valid, the blocks will not be able to move out of the game board
...
#Display text on the window screen before starting the game
...
#The input parameter is the grid returned from the create_grid() function, 
#which draws alternating lines to create cells in the game frame.
...
#all positions in a row are filled then that row will be deleted
